...
`ethereum.gray_glacier.state` to use an optimized database backed state.
"""
import logging
...
from ethereum.gray_glacier.fork_types import Account, Address, Root
...
def state_root(state: State) -> Root:
    """
    See `ethereum.gray_glacier.state`.
    """
    if state.tx_restore_points:
        raise Exception("In a non-db transaction")
    flush(state)
